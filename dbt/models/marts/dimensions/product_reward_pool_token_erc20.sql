{{
  config(
    ...
    engine='MergeTree()',
    order_by=['chain_id', 'reward_pool_address', 'token_address'],
  )
}}
...
-- Expands earned_token_addresses array from gov_vaults and joins with token_erc20

WITH reward_pool_expanded_tokens AS (
  SELECT
    vaults.earn_contract_address as reward_pool_address,
    vaults.network,
    assumeNotNull({{ evm_address('earned_token_addr') }}) as token_address
